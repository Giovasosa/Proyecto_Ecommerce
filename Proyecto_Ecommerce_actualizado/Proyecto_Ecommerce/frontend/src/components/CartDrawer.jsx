  ...
  const handleCheckout = async () => {
    try {
      ...
      const orderData = {
        ...
        last_name: user?.last_name || "Prueba",
        email: user?.email || "test_user_123@testuser.com",
        phone: "0981000000",
      ...
      // Si el usuario está logueado, mandamos el token para que la orden quede
      // asociada a su cuenta y aparezca en "Mis Pedidos".
      const fetcher = isAuthenticated ? authFetch : fetch;

      const response = await fetcher('http://127.0.0.1:8000/api/checkout/', {
        method: 'POST',
        headers: {
